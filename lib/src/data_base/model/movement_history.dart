...
Future<List<Map<String,Object?>>> getDifficultyCount() {
  String query = 'SELECT fm.difficulty AS name,COUNT(fm.difficulty) AS difficulty_count FROM movement_history AS mh JOIN blocks AS b ON b.id = mh.blocks_id JOIN wods AS w ON w.id = b.wod_id JOIN fitness_moves AS fm ON fm.id = mh.fitness_move_id WHERE expected_training_day >= "2022-12-26" GROUP BY fm.difficulty';
